...
query_result = db_env.execute_query(result.query)

print(f"Query: {result.query}")
print(f"Rows Returned: {len(query_result)}")
print(f"Execution Time: {query_result.execution_time}ms")
```
...
print(f"Avg Query Time: {results.avg_execution_time:.0f}ms")
