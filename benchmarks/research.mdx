...
```python
# Access research evaluation metrics
metrics = engine.evaluator.get_metrics()

print(f"Research Idea Generated: {metrics.get('research_idea_generated', False)}")
print(f"Novelty Score: {metrics.get('research_novelty', 0)}")
print(f"Feasibility Score: {metrics.get('research_feasibility', 0)}")
